...
if($_SERVER['REQUEST_METHOD']=='GET'){
    // $flatno=$row[flat_no];
       $flat_no=$_GET["rn"];
        $sql="SELECT price FROM `facilities` WHERE department='dance';";
        $result=$con->query($sql);
        $row=$result->fetch_assoc();
        if(!$row){
           // header("location:update_resident.php");
            header("location:user_Dance.php");
             exit;
        }
        $sql="SELECT name FROM resident WHERE flat_no=$flat_no";
        $result=$con->query($sql);
        $row1=$result->fetch_assoc();
        $sql="SELECT duration FROM `facilities` WHERE department='dance';";
        $result=$con->query($sql);
...
<body>
    ...
    <h1>Flat<span>Easy</span><strong><?php echo ($row1['name']);?></strong></h1>
        <br>
    <div class="heading">
       YOU HAVE NO ACCESS TO DANCE     <br>  
     CLICK ON THE BELOW LINK TO HAVE ACCESS
     <p class="price">PRICE   -  <?php echo $row['price'];?></p>
     <p class="price">DURATION   -  <?php echo $row2['duration'];?></p>
        <a href="dance_acc.php?rn=<?php echo $flat_no;?>" class="btn">DANCE CLASS ACCESS</a>
     </div>
</body>
</html>